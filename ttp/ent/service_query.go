...
// QueryTaserver chains the current query on the "taserver" edge.
func (sq *ServiceQuery) QueryTaserver() *TAServerQuery {
...
// QueryTacode chains the current query on the "tacode" edge.
func (sq *ServiceQuery) QueryTacode() *TACodeQuery {
	query := (&TACodeClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *ServiceQuery) Clone() *ServiceQuery {
	...
	return &ServiceQuery{
		...
		withTaserver: sq.withTaserver.Clone(),
		withTacode:   sq.withTacode.Clone(),
...
// WithTaserver tells the query-builder to eager-load the nodes that are connected to
// the "taserver" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ServiceQuery) WithTaserver(opts ...func(*TAServerQuery)) *ServiceQuery {
	query := (&TAServerClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withTaserver = query
	return sq
}

// WithTacode tells the query-builder to eager-load the nodes that are connected to
// the "tacode" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *ServiceQuery) WithTacode(opts ...func(*TACodeQuery)) *ServiceQuery {
	query := (&TACodeClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *ServiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Service, error) {
	...
	if query := sq.withTaserver; query != nil {
		if err := sq.loadTaserver(ctx, query, nodes,
			func(n *Service) { n.Edges.Taserver = []*TAServer{} },
			func(n *Service, e *TAServer) { n.Edges.Taserver = append(n.Edges.Taserver, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withTacode; query != nil {
		if err := sq.loadTacode(ctx, query, nodes,
			func(n *Service) { n.Edges.Tacode = []*TACode{} },
			func(n *Service, e *TACode) { n.Edges.Tacode = append(n.Edges.Tacode, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ServiceQuery) loadTaserver(ctx context.Context, query *TAServerQuery, nodes []*Service, init func(*Service), assign func(*Service, *TAServer)) error {
	...
	query.Where(predicate.TAServer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.TaserverColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.ta_server_service
		if fk == nil {
			return fmt.Errorf(`foreign-key "ta_server_service" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "ta_server_service" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *ServiceQuery) loadTacode(ctx context.Context, query *TACodeQuery, nodes []*Service, init func(*Service), assign func(*Service, *TACode)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Service)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.TACode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(service.TacodeColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.ta_code_service
		if fk == nil {
			return fmt.Errorf(`foreign-key "ta_code_service" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "ta_code_service" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
