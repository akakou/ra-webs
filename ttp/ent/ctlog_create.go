...
func (clc *CTLogCreate) SetDomain(s string) *CTLogCreate {
	clc.mutation.SetDomain(s)
	return clc
}
...
func (clc *CTLogCreate) SetTaInfoID(id int) *CTLogCreate {
	clc.mutation.SetTaInfoID(id)
	return clc
}

// SetNillableTaInfoID sets the "ta_info" edge to the TAInfo entity by ID if the given value is not nil.
func (clc *CTLogCreate) SetNillableTaInfoID(id *int) *CTLogCreate {
	if id != nil {
		clc = clc.SetTaInfoID(*id)
	}
	return clc
}

// SetTaInfo sets the "ta_info" edge to the TAInfo entity.
func (clc *CTLogCreate) SetTaInfo(t *TAInfo) *CTLogCreate {
	return clc.SetTaInfoID(t.ID)
}
...
func (clc *CTLogCreate) check() error {
	if _, ok := clc.mutation.Domain(); !ok {
		return &ValidationError{Name: "domain", err: errors.New(`ent: missing required field "CTLog.domain"`)}
	}
...
func (clc *CTLogCreate) createSpec() (*CTLog, *sqlgraph.CreateSpec) {
	...
	if value, ok := clc.mutation.Domain(); ok {
	...
	if nodes := clc.mutation.TaInfoIDs(); len(nodes) > 0 {
