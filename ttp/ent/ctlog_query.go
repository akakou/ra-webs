...
func (clq *CTLogQuery) QueryTaInfo() *TAInfoQuery {
	query := (&TAInfoClient{config: clq.config}).Query()
...
func (clq *CTLogQuery) Clone() *CTLogQuery {
	...
	return &CTLogQuery{
		...
		withTaInfo: clq.withTaInfo.Clone(),
...
func (clq *CTLogQuery) WithTaInfo(opts ...func(*TAInfoQuery)) *CTLogQuery {
	query := (&TAInfoClient{config: clq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	clq.withTaInfo = query
	return clq
}
...
func (clq *CTLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*CTLog, error) {
	...
	if query := clq.withTaInfo; query != nil {
		if err := clq.loadTaInfo(ctx, query, nodes, nil,
			func(n *CTLog, e *TAInfo) { n.Edges.TaInfo = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (clq *CTLogQuery) loadTaInfo(ctx context.Context, query *TAInfoQuery, nodes []*CTLog, init func(*CTLog), assign func(*CTLog, *TAInfo)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*CTLog)
	for i := range nodes {
		if nodes[i].ct_log_ta_info == nil {
			continue
		}
		fk := *nodes[i].ct_log_ta_info
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "ct_log_ta_info" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
